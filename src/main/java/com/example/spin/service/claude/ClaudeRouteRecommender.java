...
public class ClaudeRouteRecommender {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            방문 순서가 있는 루트 대안을 정확히 3개(options) 추천하세요. 각 루트는 보통 \
            2~4곳의 가게로 구성됩니다.
            반드시 후보 목록에 있는 id만 사용하고, 목록에 없는 가게를 지어내지 마세요.
            3개의 루트는 사용자가 비교해서 고를 수 있도록 서로 뚜렷하게 달라야 합니다. \
            가능하면 세 루트에 겹치는 가게가 하나도 없게 서로 다른 가게 조합으로 구성하고, \
            후보 수가 적어 어쩔 수 없는 경우에만 일부 겹쳐도 됩니다.
            아래 원칙들은 3개 루트 각각에 모두 동일하게 적용됩니다.
            ...
            하나의 루트에는 식사 가게를 한 곳만 포함하세요. 나머지 자리는 카페, 디저트, 술집, 소품샵처럼 \
